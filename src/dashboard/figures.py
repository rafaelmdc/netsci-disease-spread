...
from src.viz.compare_html import (
    deaths_averted_figure,
    dose_response_figure,
    equity_figure,
    region_spectrum_figure,
...
def aggregate_context() -> dict:
    ...
    strategy_div = spectrum_div = dose_div = deaths_div = equity_div = None
    ...
    if summary_path.exists():
        ...
        if not summary.empty:
            ...
            dose = dose_response_figure(summary)  # the paper's dose-response
            dose_div = _div(dose) if dose is not None else None
    ...
    # deaths averted (lethal type) + protection equity: pre-generated tables from
    # the aggregate task, so this view never simulates. Absent until it's run.
    deaths_path = RESULTS / "deaths.parquet"
    if deaths_path.exists():
        fig = deaths_averted_figure(pd.read_parquet(deaths_path), summary)
        deaths_div = _div(fig) if fig is not None else None
    equity_path = RESULTS / "equity.parquet"
    if equity_path.exists():
        fig = equity_figure(pd.read_parquet(equity_path))
        equity_div = _div(fig) if fig is not None else None
    ...
    return {
        ...
        "dose_div": dose_div,
        "deaths_div": deaths_div,
        "equity_div": equity_div,
        "spectrum_div": spectrum_div,
        "interdiction": interdiction,
        "has_any": bool(strategy_div or dose_div or deaths_div or equity_div
                        or spectrum_div or interdiction),
    }
