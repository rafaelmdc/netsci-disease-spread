...
\subsection{Metapopulation dynamics with two mobility mechanisms}
Each city carries a full compartmental model~\cite{colizza:reactiondiffusion};
the four models span permanent immunity (SIR: measles), no immunity (SIS:
common cold), latency (SEIR: COVID-19, SARS) and isolation (SQIR: Ebola).
Crucially, mobility is \emph{not} all of one kind, and treating it as such is
a known error~\cite{balcan:recurrent}. We therefore separate two mechanisms:

\textbf{Diffusive mobility (air, water).} A fraction of every compartment
relocates along outbound edges each day and mixes at the destination, split
by edge weight and a per-layer travel rate. This is the standard
reaction--diffusion coupling and carries a global invasion
threshold~\cite{colizza:invasion}.

\textbf{Recurrent mobility (land/commuting).} Commuters do not relocate: they
travel to a neighbour by day, mix there, and return home at
night~\cite{balcan:recurrent,sorianopanos:multiplex}. We implement this as a
coupling of the \emph{force of infection} rather than a relocation: the
pressure on city $i$'s residents is
$\pi_i=\sum_j C_{ij}\, I^{*}_j/N^{*}_j$, where $C$ is the per-capita commuting
matrix (the land radiation kernel with a stay-home diagonal) and
$I^{*}_j=\sum_i C_{ij} I_i$, $N^{*}_j=\sum_i C_{ij} N_i$ are the infectious and
total people \emph{present} at $j$. With no land layer $C$ is the identity and
$\pi_i=I_i/N_i$, recovering the plain diffusive model.

This distinction is one of modelling correctness rather than of magnitude:
recurrent commuting couples neighbours without permanently relocating residents,
which is the appropriate representation of daily commuting~\cite{balcan:recurrent}.
At our operating point its effect on the headline number is small, because the
air layer already synchronises the network: on the European air$+$land network
(SIR, $\beta=0.32$, $\gamma=0.12$) the peak active infection is $\sim$71\,M under
recurrent coupling versus $\sim$73\,M under diffusive, and $\sim$459\,M versus
$\sim$462\,M at world scale, both just below the fully synchronised SIR ceiling
of $\sim$26\% of the population at this reproduction number. The two operators
diverge in the expected direction (diffusion peaks higher and earlier) only once
the air layer is removed, and even then by under $\sim$20\%.

\textbf{In-transit transmission (refinement axis).} Optionally, travellers can
be infected \emph{during} the journey, which matters most for long confined
trips such as ferries~\cite{rocklov:diamondprincess}. We model this by running
the \emph{full} compartmental reaction on each travelling cohort for the trip
duration (distance$/$speed) at an onboard contact rate --- so passengers can be
infected, recover, incubate or be quarantined en route, not merely toggled
infected. An onboard-control parameter ($0$--$1$) reduces the onboard
transmission, representing screening, onboard quarantine or a hospital ship, so
the same machinery answers ``how well do onboard measures work?''. This is a
toggleable refinement: with it off, transmission occurs only at cities.
