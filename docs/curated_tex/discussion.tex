...
\needspace{3\baselineskip}% keep this heading off a page foot; nudge only, no full gap
...
network can partly route around. Concretely, the difference lives in the anomalous gateways: the
low-degree, high-betweenness bridge cities that a degree ranking places too low to protect, even
though a disproportionate share of cross-continental transmission must pass through
them~\cite{guimera:anomalous}. Degree and betweenness agree everywhere else, so these gateways are
the whole of betweenness's margin, and the result is really a statement about them: the rules that
win are the ones that find the bridges that traffic alone hides.

The realism ladder does not change this ordering, but it raises the stakes. Immunising the same
$15$ cities cuts the peak by about $19\%$ on the air-only network and only $12\%$ on the full
multilayer, with degree falling in step ($12\%$ to $8\%$): the fixed budget covers a smaller share
of the larger network, and its land and ferry layers give the outbreak redundant paths around any
removed city, so every rule loses absolute grip while betweenness keeps its roughly $50\%$ relative
advantage over degree~\cite{sorianopanos:multiplex}. The multilayer substrate thus makes node
targeting harder, not easier, through the very redundancy that also lets outbreaks survive route
closure (Section~\ref{sec:res-interdict}); a single-layer model, seeing only air routes, would
\emph{overstate} how much any vaccination campaign achieves.
...
case-fatality ($\mu\approx0.71$); we therefore also report deaths averted per capita for the lethal
type (Section~\ref{sec:res-deaths}), where bridge-targeting shows its largest margin. Second, every
configuration is run as a ten-seed ensemble and we attach a $95\%$ confidence interval to each
headline number, so the reductions carry uncertainty bounds and the strategy \emph{ordering} is
resolved against seed variation; a finer coverage grid and full transition-level stochasticity
remain natural extensions. Third, parameters are
